...
exports.detectCrawler = detectCrawler;
exports.extractRequestOrigin = extractRequestOrigin;
const crawler_signatures_1 = require("./crawler-signatures");
...
function detectCrawler(request) {
    const userAgent = getHeaderValue(request.headers["user-agent"]);
    const matchedSignals = [];
    const behavioralSignals = [];
    const requestPatternAnalysis = [];
    const detectedCrawler = userAgent
        ? crawler_signatures_1.crawlerSignatures.find(({ pattern }) => pattern.test(userAgent))
        : null;
    if (detectedCrawler) {
    ...
    if (!hasAuthorization)
        behavioralSignals.push("no-authorization-header");
    if (!hasCookies)
        behavioralSignals.push("no-cookies");
    if (!acceptLanguage)
        behavioralSignals.push("no-accept-language (browsers always send this)");
    if (!acceptEncoding)
        behavioralSignals.push("no-accept-encoding (browsers always send this)");
    if (!secFetchMode && !secFetchSite)
        behavioralSignals.push("no-sec-fetch-* headers (modern browsers always send these)");
    if (acceptHeader === "*/*")
        behavioralSignals.push("accept:*/* (generic accept header, typical of bots/scripts)");
    if (connection && connection.toLowerCase() === "close")
        behavioralSignals.push("connection:close (bots often close connections immediately)");
    if (userAgent) {
        if (/^axios\//i.test(userAgent))
            behavioralSignals.push("user-agent is axios HTTP client (server-side request, not a browser)");
        else if (/^node-fetch\//i.test(userAgent) || /^node\//i.test(userAgent))
            behavioralSignals.push("user-agent is Node.js HTTP client (server-side request)");
        else if (/^python-requests\//i.test(userAgent) || /^python-urllib\//i.test(userAgent))
            behavioralSignals.push("user-agent is Python HTTP client (likely a script or scraper)");
        else if (/^curl\//i.test(userAgent))
            behavioralSignals.push("user-agent is curl (command-line HTTP client)");
        else if (/^wget\//i.test(userAgent))
            behavioralSignals.push("user-agent is wget (command-line download tool)");
        else if (/^Go-http-client\//i.test(userAgent))
            behavioralSignals.push("user-agent is Go HTTP client (server-side or tool request)");
        else if (/^Java\//i.test(userAgent))
            behavioralSignals.push("user-agent is Java HTTP client (server-side application)");
    }
    else {
    ...
    // Confidence level
    const suspiciousBehavioralCount = behavioralSignals.filter((s) => s.includes("no-accept-language") ||
        s.includes("no-sec-fetch") ||
        s.includes("no-user-agent") ||
        s.includes("accept:*/*") ||
        s.includes("HTTP client") ||
        s.includes("curl") ||
        s.includes("wget")).length;
    let confidenceLevel;
    let isCrawler;
    ...
    if (forwardedFor)
        matchedSignals.push(`forwarded-for:${forwardedFor}`);
